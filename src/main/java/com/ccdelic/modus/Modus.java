 ...
 * rarity/spawn-disabling, structure block replacement, and biome size fully configurable.
 * <p>
 * Every feature is opt-in: {@link OreConfig}, {@link MobConfig}, {@link XpConfig},
 * {@link StructureBlacklistConfig} and {@link com.ccdelic.modus.config.BiomeSizeConfig} all
 * default their master switch to {@code false}, so a fresh install with no config changes
 * plays identically to vanilla. The one exception is per-structure rarity
 * ({@link StructureRarityRegistry}), whose default ({@code enabled = true, rarity = 1.0}) is
 * already a no-op in its own right - {@code false} there means "actively remove this
 * structure", not "Modus is doing something", so flipping it to match the others would have
 * made structures vanish by default instead of leaving them alone.
 * <p>
...
public class Modus {
    ...
    public Modus(IEventBus modEventBus, ModContainer modContainer) {
        ...
        modContainer.registerConfig(ModConfig.Type.COMMON, BiomeSizeConfig.SPEC, CONFIG_FOLDER + "/biomeSize/biomeSize.toml");

        LOGGER.info(
            "Modus loaded - ore generation, mob spawn caps, XP gain rate, structure rarity, structure block "
                + "replacement and biome size are now driven by config"
        );
    }
}
