 ...
 * UI to fit.
 * <p>
 * Everything else (add/undo buttons, saving {@link #cfgList} back to the underlying
 ...
 * edited is different, plus {@link #createResetButton()} adds a {@link ResetConfirmation}
 * prompt in front of NeoForge's own (otherwise instant, no-confirmation) list reset.
 */
public class BlacklistListScreen extends ConfigurationScreen.ConfigurationListScreen<String> {
    ...
    /**
     * Gates NeoForge's stock (instant, no-confirmation) list reset behind a
     * {@link ResetConfirmation} Yes/No prompt - the reset logic itself, built by
     * {@code super.createResetButton()}, is untouched; only the button that triggers it is
     * replaced with one that confirms first.
     */
    @Override
