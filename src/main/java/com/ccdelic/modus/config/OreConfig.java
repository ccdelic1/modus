...
public final class OreConfig {
    ...
    /**
     * Ores that have already logged a {@link OreEntry#clampedHeightRange} degenerate-collapse
     * warning this session - so that warning fires once per ore, ever, rather than once per
     * vein-position roll (a genuinely hot path). Thread-safe since world generation can invoke
     * this from multiple chunk-generation worker threads concurrently.
     */
    private static final Set<OreType> loggedDegenerateHeightRange = ConcurrentHashMap.newKeySet();
    ...
    public record OreEntry(
        ...
        public int rollVeinSize(net.minecraft.util.RandomSource random) {
            // Each value is read exactly once into a local: a config reload landing between
            // two separate get() calls on the SAME field could otherwise let the lo and hi
            // computations below each see a different generation of that field's value (e.g.
            // lo computed from the old veinSizeMin, hi from a reload's new one), a torn read
            // within one field rather than just a cross-field race.
            int configuredMin = this.veinSizeMin.get();
            int configuredMax = this.veinSizeMax.get();
            int lo = Math.max(0, Math.min(configuredMin, configuredMax));
            int hi = Math.min(MAX_SAFE_VEIN_SIZE, Math.max(configuredMin, configuredMax));
            // No hi < lo guard needed here (unlike clampedHeightRange below): lo and hi are
            // clamped into the FIXED, always-well-formed [0, MAX_SAFE_VEIN_SIZE] range (0 is
            // never greater than MAX_SAFE_VEIN_SIZE), so lo = min(configuredMin, configuredMax)
            // and hi = max(configuredMin, configuredMax) provably satisfy lo <= hi for every
            // reachable configuredMin/configuredMax (both are themselves already constrained to
            // [0, MAX_SAFE_VEIN_SIZE] by this same ModConfigSpec range). clampedHeightRange's
            // guard is NOT redundant the same way: it clamps into genMin/genMax, a range
            // supplied by the current generation context that CAN itself be inverted (e.g. a
            // generator reporting zero height), and Mth.clamp does not preserve a <= relationship
            // across an inverted [min, max] window - see that method for the worked-through case.
        ...
        public int[] clampedHeightRange(int genMin, int genMax) {
            // Same reasoning as rollVeinSize: read each field exactly once so a config reload
            // landing mid-call can't let the lo and hi computations below see two different
            // generations of the same field's value.
            int configuredMin = this.minHeight.get();
            int configuredMax = this.maxHeight.get();
            int lo = Math.min(configuredMin, configuredMax);
            int hi = Math.max(configuredMin, configuredMax);
            lo = net.minecraft.util.Mth.clamp(lo, genMin, genMax);
            hi = net.minecraft.util.Mth.clamp(hi, genMin, genMax);
            // Unlike rollVeinSize's analogous-looking (and there, provably dead) guard, this
            // one is load-bearing: genMin/genMax come from the current generation context and
            // are NOT guaranteed genMin <= genMax (e.g. a generator reporting zero height depth
            // gives genMax = genMin - 1). Mth.clamp(value, min, max) is only order-preserving
            // (x <= y implies clamp(x) <= clamp(y)) when min <= max; for an inverted [min, max]
            // window it is not - e.g. clamp(3, 10, 5) = 10 but clamp(20, 10, 5) = 5, so a
            // pre-clamp lo <= hi can still come out the other way round after clamping into an
            // inverted range. This guard is what keeps ConfigurableHeightRangePlacement's
            // random.nextInt(range[1] - range[0] + 1) call from ever seeing a non-positive bound.
            if (hi < lo) {
                hi = lo;
                // This should be unreachable for any normal Overworld/Nether/End generator (all
                // report a positive height depth) - so if it ever fires, that's a genuinely
                // useful signal that this dimension's generator is reporting an inverted or
                // zero-height range, worth a human looking into. Logged once per ore, ever,
                // rather than once per vein-position roll (this method runs on that hot path).
                if (loggedDegenerateHeightRange.add(this.ore())) {
                    Modus.LOGGER.warn(
                        "Modus: the current generation context reports an inverted or zero-height Y range "
                            + "(min={}, max={}) for {} ore; its configured height range has collapsed to a "
                            + "single Y level ({}) to avoid a crash. This should only happen for an unusual "
                            + "custom dimension generator, never a normal Overworld/Nether/End.",
                        genMin, genMax, this.ore().getSerializedName(), lo
                    );
                }
            }
