 ...
 * server start. {@link MobCategory#getMaxInstancesPerChunk()} has no vanilla setter, so this
 * reflectively overwrites the enum constant's backing field - safe here because the field is
 * a non-static instance field (not a compile-time-inlined constant) and, on NeoForge's
 * classpath-based mod loading, not subject to JPMS module access restrictions.
 * <p>
 ...
 * <p>
 * NeoForge 1.21.1 ships official Mojang mappings at runtime, so the field is simply named
 * "max" rather than an obfuscated name that would need to be looked up.
 * <p>
