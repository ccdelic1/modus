 ...
 * <p>
 * {@code doPlace}'s {@code setBlockState(rx, ry, rz, target.state, false)} update flag is
 * deliberately NOT changed to match {@link ConfigurableScatteredOreFeature}'s
 * {@code setBlock(..., 2)}: confirmed by reading both in the decompiled vanilla source,
 * vanilla's own {@link OreFeature#doPlace} and {@code ScatteredOreFeature#place} already use
 * these exact same two different flags for their respective feature types (flag {@code false},
 * sending no block update, here; flag {@code 2}, sending one, there) - this class faithfully
 * mirrors that, rather than introducing its own inconsistency. Unifying these flags would be a
 * genuine world-generation behavior change (whether neighbor block updates, redstone/fluid/
 * gravity reactions, and other mods' block-change listeners fire during chunk generation), with
 * no vanilla precedent for which of the two behaviors would be "more correct" to converge on -
 * so, deliberately, this stays exactly as faithful to vanilla as the rest of this port.
 */
public class ConfigurableOreFeature extends Feature<ConfigurableOreConfiguration> {
    ...
    private static boolean doPlace(
        ...
        // The per-cell index below (`i2 - x + (j2 - y) * width + (k2 - z) * width * height`) is
        // always strictly less than this same width*height*width product, so validating the
        // product fits safely in an int - here, using widening long arithmetic so the check
        // itself can't be fooled by the very overflow it's guarding against - is sufficient to
        // guarantee every index derived from it fits too, without needing to touch that hot
        // per-voxel arithmetic at all. Only a vein size far beyond OreConfig#MAX_SAFE_VEIN_SIZE's
        // documented safe limit could ever reach this, but int overflow would otherwise silently
        // wrap into a negative BitSet index and crash chunk generation - so this fails loudly and
        // gracefully (skip this vein) well before that arithmetic ever gets a chance to overflow.
        long capacity = (long) width * height * width;
        if (capacity > Integer.MAX_VALUE) {
            Modus.LOGGER.error(
                "Modus: ore vein bounding volume ({} cells) is too large to place safely; skipping this vein. "
                    + "This should only be possible if OreConfig.MAX_SAFE_VEIN_SIZE has been raised far beyond "
                    + "its documented safe limit.",
                capacity
            );
            return false;
        }
        ...
        int placed = 0;
        BitSet bitset = new BitSet((int) capacity);
